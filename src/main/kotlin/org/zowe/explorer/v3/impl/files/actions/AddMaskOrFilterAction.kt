...
/**
 * Action to add a dataset mask or USS filter to an existing files profile.
 * Visible only when a [FilesProfileNodeDescriptor] node is selected
 */
class AddMaskOrFilterAction : DumbAwareEDTAction() {
  override fun actionPerformed(e: AnActionEvent) {
    ...
    val selectedNode = ExplorerTreeComponentService.getService()
      ...
      .selectedNodes
      .singleOrNull() ?: return
    val profileDescriptor = selectedNode.nodeDescriptor as? FilesProfileNodeDescriptor ?: return
    val configType = ZoweConfigService.getService().getSelectedConfigType(project)
    AddMaskOrFilterDialog(project, configType, profileDescriptor.displayName).show()
  }
