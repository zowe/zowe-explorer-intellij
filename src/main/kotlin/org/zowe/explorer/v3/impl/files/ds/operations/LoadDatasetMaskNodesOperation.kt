...
import org.zowe.explorer.v3.impl.files.ds.tree.nodes.PartitionedDatasetNodeDescriptor
...
class LoadDatasetMaskNodesOperation(
  ...
  override suspend fun fetchChildren(): List<ExplorerTreeNode> {
    ...
    return if (listDataSetsResponse.status.type != StatusType.SUCCESS) {
      listOf(
        ExplorerTreeNode(
          ErrorNodeDescriptor("Host returned ${listDataSetsResponse.status.text}"),
