...
import org.zowe.explorer.v3.impl.files.dialogs.DeleteMaskOrFilterHandler
...
/**
 * Action to delete an existing dataset mask or USS filter.
 ...
 */
class DeleteMaskOrFilterAction : DumbAwareEDTAction() {
  ...
  override fun actionPerformed(e: AnActionEvent) {
    ...
    val (entryType, value) = resolveEntryInfo(selectedNode) ?: return
    val profileName = resolveProfileName(selectedNode) ?: return

    val typeName = entryType.displayName.lowercase()
    val result = Messages.showYesNoDialog(
      project,
      "Are you sure you want to delete $typeName '$value'?",
      "Delete ${entryType.displayName}",
    ...
    try {
      ...
      DeleteMaskOrFilterHandler(project.basePath, project)
        .deleteEntry(configType, profileName, entryType, value)
    } catch (ex: Exception) {
      Messages.showErrorDialog(project, ex.message ?: "Unknown error", "Failed to Delete ${entryType.displayName}")
    }
  }
