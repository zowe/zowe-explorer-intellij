...
//Errors messages
const val errorDatasetNameInvalid = "Each name segment (qualifier) is 1 to 8 characters, the first of which must be alphabetic (A to Z) or " +
...
const val errorRenameMemberFailed = "Rename member failed"
const val errorMemberInUse = "Member in use"
const val errorInvalidMemberNameMessage = "Member name should contain only A-Z a-z 0-9 or national characters"
const val errorInvalidBeginningOfMemberName = "Member name should start with A-Z a-z or national characters"
const val errorEmptyField = "This field must not be blank"
const val errorDatasetInvalidSection = "Each name segment (qualifier) is 1 to 8 characters, the first of which must be alphabetic (A to Z) or national (# @ \$). The remaining seven characters are either alphabetic, numeric (0 - 9), national, a hyphen (-). Name segments are separated by a period (.)"
const val errorInvalidDatasetNameLength = "Dataset name cannot exceed 44 characters"
//const val ERROR_IN_PLUGIN = "Error in plugin For Mainframe"
const val errorDatasetRenameFailed = "Data set rename failed"
...
var errorHostUnknowable = "Этот хост неизвестен (%s)"
...
const val errorInvalidLrecFromBack = "Cannot allocate dataset A23.VALID on CON00001 Code: 500 CATEGORY: 1 MESSAGE: For a V file, the LRECL must be greater than 4 bytes. RETURN CODE: 4 DETAILS: null REASON: 13.0 "
...
fun formAllocationParams(
  name: String = "A23.VALID",
  preset: String = "Custom Dataset",
  dsOrg: DsOrg = DsOrg.PO,
  unit: AllocUnit = AllocUnit.TRK,
  primAlloc: String = "10",
  secAlloc: String = "1",
...
): AllocateDatasetParams {
  return AllocateDatasetParams(
    name = name,
    preset = preset,
    dsOrg = dsOrg,
    unit = unit,
...
val textInBlockLengthParams = Pair(formAllocationParams(secAlloc = "0", avgBlkLen = "Add"), enterNumberOnly)
val specSymbolsInBlockLengthParams = Pair(formAllocationParams(secAlloc = "0",blksz="#!@#$%", avgBlkLen = "1"),
  enterNumberOnly)
val textInBlockSizeParams = Pair(formAllocationParams(secAlloc = "0", blksz="Abc"), enterNumberOnly)
...
val specSymbolsInAllocationParams = Pair(formAllocationParams(primAlloc = "#@/\$%^&*"), enterNumberOnly)
...
val invalidLrecInCustom = Pair(formAllocationParams(lrecl="1", recfm=RecFM.V),
  errorInvalidLrecFromBack)
val invalidLrecInSequence = Pair(formAllocationParams(lrecl="1", preset="Sequential Dataset", recfm=RecFM.V),
  errorInvalidLrecFromBack)
val invalidLrecInPds = Pair(formAllocationParams(recfm=RecFM.V,lrecl="1"),
  errorInvalidLrecFromBack)
val invalidLrecInPdwWithMember = Pair(formAllocationParams(preset="PDS with empty member Dataset", memberName="TEST", lrecl="1"),
  errorInvalidLrecFromBack)
val invalidLrecInPdsWithJcl = Pair(formAllocationParams(recfm=RecFM.V, memberName="TEST", lrecl="1", preset="PDS with sample JCL member Dataset"),
errorInvalidLrecFromBack)
...
val incorrectRenameMember = mapOf(
    ...
    Pair(INVALID_NAME_VIA_CONTEXT_MENU, errorInvalidMemberNameMessage),
    Pair(INVALID_INVALID_FIRST_SYMBOL, errorInvalidBeginningOfMemberName),
    Pair(EMPTY_STRING, errorEmptyField),
)
