...
class ExplorerPasteProviderTestSpec : AppInitShouldSpec("explorer/ui/ExplorerPasteProvider", {
  context("perform paste via paste provider") {
    ...
    val mockedCopyPasterProvider = mockk<FileExplorerView.ExplorerCopyPasteSupport> {
      every { project } returns mockedProject
      every { bufferLock } returns mockk {
        every {
          lock()
        } answers {
      ...
      every { getSourceFilesFromClipboard() } returns mockedClipboardBuffer
    }
    val mockedFileExplorerView = mockk<FileExplorerView> {
      every { isCut } returns AtomicBoolean(true)
      every { ignoreVFSChangeEvents } returns AtomicBoolean(false)
      every { copyPasteSupport } returns mockedCopyPasterProvider
      every { explorer } returns mockk<AbstractExplorerBase<ConnectionConfig, FilesWorkingSet, FilesWorkingSetConfig>>()
    }

    val mockedExplorerPasteProvider = spyk(ExplorerPasteProvider(), recordPrivateCalls = true)
    ...
    beforeEach {
      ...
      every { nodeToRefreshSource.virtualFile } returns mockk<MFVirtualFile> {
        every { parent } returns mockk {
          every { parent } returns null
        }
        every { fileSystem } returns mockk<MFVirtualFileSystem> {
          every { model } returns mockk<MFVirtualFileSystemModel> {
            every { deleteFile(any(), any()) } just Runs
          }
        }
      }
      every { nodeToRefreshTarget.parent } returns nodeToRefreshTarget
      every { nodeToRefreshTarget.path } returns mockk<TreePath>()
      every { nodeToRefreshTarget.virtualFile } returns mockk<MFVirtualFile> {
        every { parent } returns mockk {
          every { parent } returns null
        }
        every { fileSystem } returns mockk<MFVirtualFileSystem> {
      ...
      every {
        mockedFileExplorerView.getNodesByFile(any<VirtualFile>())
      } returns listOf(nodeToRefreshSource, nodeToRefreshTarget)

      every {
        dataOpsManagerService.getNameResolver(any<VirtualFile>(), any<VirtualFile>())
      } returns DefaultNameResolver()
      every {
        dataOpsManagerService.tryToGetAttributes(any<VirtualFile>())
      } returns mockk()
      every {
        dataOpsManagerService.performOperation(any<Operation<Any>>(), any<ProgressIndicator>())
      } returns Unit
      ...
      every {
        nodeToRefreshSource.cleanCache(cleanBatchedQuery = true)
      } answers {
        isPastePerformed = true
      }
      every {
        nodeToRefreshTarget.cleanCache(cleanBatchedQuery = true)
      } answers {
        isPastePerformed = true
      }
    ...
    should("perform paste without conflicts USS file -> USS folder") {
      ...
      every { dataOpsManagerService.tryToGetAttributes(childDestinationVirtualFile) } returns childDestFileAttributes
      every { dataOpsManagerService.tryToGetAttributes(mockedSourceFile) } returns mockedSourceAttributes
      every { dataOpsManagerService.tryToGetAttributes(mockedTargetFile) } returns targetAttributes
      every {
        dataOpsManagerService.performOperation(any<Operation<Any>>(), any<ProgressIndicator>())
      } returns Unit
      ...
      every {
        mockedCopyPasterProvider
          .getDestinationSourceFilePairs(any<List<VirtualFile>>(), any<List<VirtualFile>>(), any<Boolean>())
      } returns mutableListOf(Pair(mockedTargetFile, mockedSourceFile))

      mockedExplorerPasteProvider.performPaste(mockedDataContext)

      assertSoftly { isPastePerformed shouldBe true }
    }
    ...
    should("perform paste without conflicts USS file -> PDS without skipping") {
      ...
      every { mockedFileExplorerView.getNodesByFile(mockedSourceFile) } returns listOf(mockedSourceNode)
      every { mockedFileExplorerView.getNodesByFile(mockedTargetFile) } returns listOf(mockedNodeTarget)
      ...
      every {
        mockedSourceNodeParent.cleanCache(cleanBatchedQuery = true)
      } answers {
        isPastePerformed = true
      }
      every {
        mockedNodeTarget.cleanCache(cleanBatchedQuery = true)
      } answers {
        isPastePerformed = true
      }
    ...
    should("perform paste without conflicts PS file -> PDS without skipping") {
      ...
      // CopyPaste node buffer
      val copyPasteNodeData = mockk<NodeData<*>> {
        every { node } returns mockedSourceNode
        every { file } returns mockedSourceFile
        every { attributes } returns mockedSourceAttributes
      }
      val copyPasteNodeDataList = listOf(copyPasteNodeData)
      val mockedCopyPasteBuffer = LinkedList(copyPasteNodeDataList)
      every { mockedCopyPasterProvider.copyPasteBuffer } returns mockedCopyPasteBuffer
    ...
    should("perform paste without conflicts USS file -> Local folder") {
      ...
      every { dataOpsManagerService.tryToGetAttributes(childDestinationVirtualFile) } returns null
      every { dataOpsManagerService.tryToGetAttributes(mockedSourceFile) } returns mockedSourceAttributes
      every { dataOpsManagerService.tryToGetAttributes(mockedTargetFile) } returns null
      every {
        dataOpsManagerService.performOperation(any<Operation<Any>>(), any<ProgressIndicator>())
      } returns Unit
    ...
    should("perform paste with conflicts USS files (>5) -> Local folder + remote but declining download") {
      ...
      every { dataOpsManagerService.tryToGetAttributes(childDestinationVirtualFile1) } returns null
      every { dataOpsManagerService.tryToGetAttributes(childDestinationVirtualFile2) } returns childDestMFFile2Attr
      every { dataOpsManagerService.tryToGetAttributes(childDestinationVirtualFile3) } returns null
      every { dataOpsManagerService.tryToGetAttributes(mockedSourceFile1) } returns mockedSourceAttributes1
      every { dataOpsManagerService.tryToGetAttributes(mockedSourceFile2) } returns mockedSourceAttributes2
      every { dataOpsManagerService.tryToGetAttributes(mockedSourceFile3) } returns mockedSourceAttributes3
      every { dataOpsManagerService.tryToGetAttributes(mockedSourceFile4) } returns mockedSourceAttributes4
      every { dataOpsManagerService.tryToGetAttributes(mockedSourceFile5) } returns mockedSourceAttributes5
      every { dataOpsManagerService.tryToGetAttributes(mockedSourceFile6) } returns mockedSourceAttributes6
      every { dataOpsManagerService.tryToGetAttributes(mockedTargetFile1) } returns null
      every { dataOpsManagerService.tryToGetAttributes(mockedTargetFile2) } returns mockedTargetFile2Attributes
      every { dataOpsManagerService.tryToGetAttributes(mockedTargetFile3) } returns null
      every {
        dataOpsManagerService.performOperation(any<Operation<Any>>(), any<ProgressIndicator>())
      } returns Unit
    ...
    should("perform paste without conflicts Local -> USS") {
      ...
      every { dataOpsManagerService.tryToGetAttributes(childDestinationVirtualFile) } returns childDestMFFileAttr
      every { dataOpsManagerService.tryToGetAttributes(mockedSourceFile) } returns mockedSourceAttributes
      every { dataOpsManagerService.tryToGetAttributes(mockedTargetFile) } returns mockedTargetFileAttributes
      every {
        dataOpsManagerService.performOperation(any<Operation<Any>>(), any<ProgressIndicator>())
      } returns Unit
    ...
    context("Test conflict resolution") {
      ...
      should("Skip 2 files one by one") {
        ...
        every { dataOpsManagerService.tryToGetAttributes(mockedSourceFile1) } returns mockedSourceAttributes1
        every { dataOpsManagerService.tryToGetAttributes(mockedSourceFile2) } returns mockedSourceAttributes2
        every { dataOpsManagerService.tryToGetAttributes(mockedTargetFile1) } returns mockedTargetAttributes1
        every { dataOpsManagerService.tryToGetAttributes(mockedTargetFile2) } returns mockedTargetAttributes2
        every { dataOpsManagerService.tryToGetAttributes(mockedTargetFolder) } returns mockedTargetFolderAttributes
        every {
          dataOpsManagerService.performOperation(any<Operation<Any>>(), any<ProgressIndicator>())
        } returns Unit
      ...
      should("Skip 1 file and 1 overwrite") {
        ...
        every { dataOpsManagerService.tryToGetAttributes(mockedTargetFolder) } returns mockedTargetFolderAttributes
        every {
          dataOpsManagerService.performOperation(any<MoveCopyOperation>(), any<ProgressIndicator>())
        } answers {
          val operation = firstArg<MoveCopyOperation>()
      ...
      should("Use new name for 1 file and 1 overwrite") {
        ...
        every { dataOpsManagerService.tryToGetAttributes(mockedTargetFile1) } returns mockedTargetAttributes1
        every { dataOpsManagerService.tryToGetAttributes(mockedTargetFile2) } returns mockedTargetAttributes2
        every { dataOpsManagerService.tryToGetAttributes(mockedTargetFile3) } returns mockedTargetAttributes3
        every { dataOpsManagerService.tryToGetAttributes(mockedTargetFolder) } returns mockedTargetFolderAttributes
        every {
          dataOpsManagerService.performOperation(any<MoveCopyOperation>(), any<ProgressIndicator>())
        } answers {
      ...
      should("Use new name for sequential dataset (moving to pds)") {
        ...
        val datasetAttributes = mockk<RemoteDatasetAttributes> {
          every { name } returns "DATASET.TEST"
          every { isDirectory } returns false
          every { isPastePossible } returns false
        }
        ...
        every { dataOpsManagerService.tryToGetAttributes(mockedSourceFile1) } returns mockedSourceAttributes1
        every { dataOpsManagerService.tryToGetAttributes(mockedTargetFile1) } returns mockedTargetAttributes1
        every { dataOpsManagerService.tryToGetAttributes(mockedTargetFolder) } returns mockk<RemoteDatasetAttributes>()
        every {
          dataOpsManagerService.performOperation(any<MoveCopyOperation>(), any<ProgressIndicator>())
        } answers {
          val operation = firstArg<MoveCopyOperation>()
      ...
      should("Resolve conflicts in source files when copying to PDS") {
        ...
        every {
          dataOpsManagerService.getNameResolver(any<VirtualFile>(), any<VirtualFile>())
        } returns NotSeqToPDSResolver(dataOpsManagerService)
        every { dataOpsManagerService.tryToGetAttributes(mockedSourceFile1) } returns mockedSourceAttributes1
        every { dataOpsManagerService.tryToGetAttributes(mockedSourceFile2) } returns mockedSourceAttributes2
        every { dataOpsManagerService.tryToGetAttributes(mockedTargetFile1) } returns mockedTargetAttributes1
        every { dataOpsManagerService.tryToGetAttributes(mockedTargetFile2) } returns mockedTargetAttributes2
      ...
      should("Resolve conflicts in source files when copying to PDS from different source directories") {
        ...
        every {
          Messages
            .showOkCancelDialog(any<Project>(), any<String>(), any<String>(), any<String>(), any<String>(), any<Icon>())
        } returns Messages.OK
        every {
          Messages.showYesNoDialog(
            any() as Project?, any() as String, any() as String, any() as String, any() as String, any() as Icon?
