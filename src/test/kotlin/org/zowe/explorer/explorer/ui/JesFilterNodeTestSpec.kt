...
class JesFilterNodeTestSpec : AppInitShouldSpec("explorer/ui/JesFilterNode", {
  ...
  context("sort children nodes") {
    ...
    every { uiComponentManager.getExplorerContentProvider(any<Class<JesExplorer>>()) } returns mockk()
    ...
    val jobNode1 = mockk<JobNode> {
      every { value } returns virtualFile1
    }
    val jobNode2 = mockk<JobNode> {
      every { value } returns virtualFile2
    }
    val jobNode3 = mockk<JobNode> {
      every { value } returns virtualFile3
    }
    ...
    val jobAttributes3 = mockk<RemoteJobAttributes> {
      every { jobInfo } returns jobInfo3
    }
