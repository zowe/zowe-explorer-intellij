...
class SyncToolbarProviderTestSpec : AppInitShouldSpec("dataops/SyncToolbarProvider", {
  context("sync action toolbar") {
    ...
    should("run activity and resolve action group toolbar when action group is not null") {
      ...
      every {
        mockedActionManagerInstance.getAction(any<String>())
      } answers {
        isResolved = true
      ...
      assertSoftly { isResolved shouldBe true }
    }
    ...
    should("run activity and resolve action group toolbar when action group is simple action") {
      ...
      every { mockedActionManagerInstance.getAction(any<String>()) } returns mockedNotActionGroupForTest
      every {
        mockedActionManagerInstance.registerAction(any<String>(), any<DefaultActionGroup>())
      } answers {
        isResolved = true
      }
